  ...
  <div class="like_sel">
    ...
    <transition name="slide" mode="out-in">
      <div class="sel_option" :class="{arrow: arrow}" v-if="active">
        <ul class="ul_list">
          <li class="col" v-for="(item,index) in list" :class="{child: item.child}" :key="index" @click="item.command || clickHandle">
            <label><em class="font_midd"><i v-if="item.icon" :class="'iconfont ' + item.icon"></i></em><span class="font_midd">{{item.name}}</span></label>
            ...
            <div v-if="item.child" class="menu_item">
              <ul class="ul_list">
                <li class="col" :class="{'is-active': item.active}" v-for="(sitem,sindex) in item.list" @click="sitem.command">{{sitem.name}}</li>
              </ul>
            </div>
