        ...
        <ul class="ul_list">
          <li class="col" v-for="(item,index) in list" :class="{child: item.child}" :key="index">
            <template v-if="item.nodes === 'table' && item.commandType">
              <label><em class="font_midd"><i v-if="item.icon" :class="'iconfont ' + item.icon"></i></em><span class="font_midd" @click="nodes.table.command({type: sitem.commandType})">{{item.name}}</span></label>
            </template>
            <template v-else>
