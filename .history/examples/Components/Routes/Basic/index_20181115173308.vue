		...
		<editor class="editor" :extensions="extensions" @update="onUpdate" ref="editor">
			<div class="menubar" slot="menubar" slot-scope="{ nodes, marks }">
				<div v-if="nodes && marks">
					...
					<button class="menubar__button" :class="{ 'is-active': nodes.table.active() }" @click="nodes.table.command({type: 'insert', options: {rows: 3,cols: 4, headerRow: false}})">
						insert Table
					</button>

					<button class="menubar__button" @click="nodes.table.command({type: 'deleteTable'})">
						delete Table
					</button>
					<button class="menubar__button" @click="nodes.table.command({type: 'splitCell'})">
						split Table
					</button>
					<button class="menubar__button" @click="nodes.table.command({type: 'mergeCells'})">
						merge Table
					</button>
					<button class="menubar__button" @click="nodes.table.command({type: 'setCellBackground'})">
						set background
					</button>
					<button class="menubar__button" @click="nodes.table.command({type: 'toggleHeaderCell'})">
						toggleHeaderCell
					</button>
