...
Final paragraph.`
  ...
  describe('Markdown Rendering', () => {
    ...
    it('should render nested marks with correct tag order', () => {
      ...
      const markdown = markdownManager.renderNodes(doc)
      // With correct closing order (not reversed), should get: ~**abcd**~
      // Opening: bold (**) then strike (~) creates: ~**abcd
      // Closing: bold (**) then strike (~) creates: ~**abcd**~
      expect(markdown).to.equal('~~**abcd**~~')
    })
