...
import {
  ...
  TableOfContentData, TableOfContentDataItem, TableOfContentsOptions, TableOfContentsStorage,
} from './types.js'
import { getHeadlineLevel, getLinearIndexes } from './utils.js'

export * from './types.js'

const addTocActiveStatesAndGetItems = (content: TableOfContentDataItem[], options: {
  editor: Editor;
...
export const TableOfContents = Extension.create<TableOfContentsOptions, TableOfContentsStorage>({
  ...
  onCreate() {
    ...
    this.storage.scrollHandler = () => {
      ...
      const scrollPosition = scrollParent instanceof HTMLElement
