...
export interface PageBreakAwareTableHeaderOptions {
  /**
  ...
  /**
   * Enable page break support for table headers
   * @default true
   */
...
/**
 * Enhanced table header extension with page break support.
 * This extension allows table header content to flow across multiple pages
 * similar to Google Docs, preventing tables from breaking when header content
 * exceeds the page height.
 */
export const PageBreakAwareTableHeader = Node.create<PageBreakAwareTableHeaderOptions>({
  name: 'pageBreakAwareTableHeader',
  ...
  renderHTML({ HTMLAttributes, node }) {
    ...
    if (this.options.enablePageBreaks) {
      const classes = ['page-break-aware-cell', 'page-break-aware-header']
  ...
  addCommands() {
    return {
      /**
       * Set page break behavior for the current table header cell
       */
      setHeaderPageBreak:
        (options: {
        ...
        ({ tr, dispatch, state }: any) => {
          ...
          // Find the table header cell node
          let cellPos: number | null = null
          for (let depth = $anchor.depth; depth > 0; depth -= 1) {
            if ($anchor.node(depth).type.spec.tableRole === 'header_cell') {
