...
| `input = new InputState`                      | L51                                     | Input/composition state. `get composing()` (L110) reads `this.input.composing`; the composition guard keys off it. Also `input.compositionNode` (L199) and `input.mouseDown` (L211).                             |
...
| `onSelectionChange()`                | bound L80, listener L120/124, impl L132 | Wrap to no-op while `view.composing`, so IME composition isn't disturbed by selectionchange reads. |
