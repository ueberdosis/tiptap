...
describe('minMax', () => {
  ...
  it('handles default parameters', () => {
    ...
    expect(minMax(5)).toBe(0)
    expect(minMax(5, 2)).toBe(0)
  })

  it('handles case where min is greater than max', () => {
    // Math.min/Math.max will still work, but the behavior might be unexpected
    expect(minMax(5, 10, 2)).toBe(2) // min(10, 2) = 2, then max(5, 2) = 5? Wait, no:
    // Actually: Math.min(Math.max(5, 10), 2) = Math.min(10, 2) = 2
    expect(minMax(5, 10, 2)).toBe(2)
  })
})
