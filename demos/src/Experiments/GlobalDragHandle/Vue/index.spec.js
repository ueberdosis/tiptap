context('/src/Experiments/GlobalDragHandle/Vue/', () => {
  beforeEach(() => {
    cy.visit('/src/Experiments/GlobalDragHandle/Vue/')
  })
  ...
  it('should show debug information', () => {
    cy.get('.debug-info').should('exist')
    cy.get('.debug-info details').should('have.length', 2)

    // Test expanding debug info
    cy.get('.debug-info details').first().find('summary').click()
    cy.get('.debug-info details').first().find('pre').should('be.visible')
  })
