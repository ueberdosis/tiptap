...
test.describe('/src/Extensions/Typography/React/', () => {
  ...
  test('should keep dates as they are', async ({ page }) => {
    ...
    await typeText(page, '1/4/2024')
    await expect(page.locator('.tiptap').filter({ hasText: '1/4/2024' }).first()).toBeAttached()
  })

  test('should make a fraction only with spaces afterwards', async ({ page }) => {
    await page.locator('.tiptap').first().click()
    await typeText(page, '1/4')
    await expect(page.locator('.tiptap').filter({ hasText: '1/4' }).first()).toBeAttached()
    await page.locator('.tiptap').first().click()
    await typeText(page, '{selectall}{backspace}1/4 ')
    await expect(page.locator('.tiptap').filter({ hasText: '¼' }).first()).toBeAttached()
  })
