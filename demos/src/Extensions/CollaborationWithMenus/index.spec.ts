...
const demoName = 'CollaborationWithMenus'
...
test.describe(`${demoPath}/${demoName}`, () => {
  frameworkPaths.forEach(frameworkPath => {
    ...
    test.describe(`${frameworkPath}`, () => {
      ...
      test('has menu plugins initiated', async ({ page }) => {
        const editor = await getEditor(page)
        await page.waitForTimeout(700)
        const result = await editor.evaluate((el: any) => {
          const plugins = el.editor.view.state.plugins
          const bubble = plugins.find((p: any) => p.spec.key?.key === 'bubbleMenu$')
          const floating = plugins.find((p: any) => p.spec.key?.key === 'floatingMenu$')
          return { bubble: !!bubble, floating: !!floating }
        })
        if (frameworkPath === 'Vue') {
          expect(result.bubble || result.floating).toBe(true)
        } else {
