...
test.describe('/src/Examples/MarkdownShortcuts/React/', () => {
  ...
  test('should make a h4', async ({ page }) => {
    await page.locator('.tiptap').first().click()
    await typeText(page, '#### Headline')
    await expect(page.locator('.tiptap').locator('h4')).toContainText('Headline')
  })

  test('should make a h5', async ({ page }) => {
    await page.locator('.tiptap').first().click()
    await typeText(page, '##### Headline')
    await expect(page.locator('.tiptap').locator('h5')).toContainText('Headline')
  })

  test('should make a h6', async ({ page }) => {
    await page.locator('.tiptap').first().click()
    await typeText(page, '###### Headline')
    await expect(page.locator('.tiptap').locator('h6')).toContainText('Headline')
  })
  ...
  test('should create a code block without language', async ({ page }) => {
    ...
    await expect(page.locator('.tiptap').locator('pre')).toContainText('const foo = bar')
  })

  test('should create a bullet list from asteriks', async ({ page }) => {
    await page.locator('.tiptap').first().click()
    await typeText(page, '* foobar')
    await expect(page.locator('.tiptap').locator('ul')).toContainText('foobar')
  })
  ...
  test('should create a ordered list', async ({ page }) => {
    ...
    await expect(page.locator('.tiptap').locator('ol')).toContainText('foobar')
  })

  test('should create a blockquote', async ({ page }) => {
    await page.locator('.tiptap').first().click()
    await typeText(page, '> foobar')
    await expect(page.locator('.tiptap').locator('blockquote')).toContainText('foobar')
  })
})
