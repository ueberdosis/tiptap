...
test.describe('/src/Examples/CustomParagraph/React/', () => {
  ...
  test('should have a paragraph and text length', async ({ page }) => {
    ...
    await expect(page.locator('.tiptap .label').first()).toBeAttached()
  ...
  test('should have new paragraph', async ({ page }) => {
    ...
    await expect(page.locator('.tiptap p').nth(1).first()).toBeAttached()
    await expect(page.locator('.tiptap p').nth(1).first()).toHaveText('')
    await expect(page.locator('.tiptap .label').nth(1).first()).toBeAttached()
    await expect(page.locator('.tiptap .label').nth(1).first()).toHaveText('0')
  })
})
